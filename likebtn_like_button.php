...
$likebtn_like_button_no_excerpts = array(
    LIKEBTN_ENTITY_PAGE,
    LIKEBTN_ENTITY_COMMENT
);
// post types titles
global $likebtn_like_button_entity_titles;
$likebtn_like_button_entity_titles = array(
    ...
    LIKEBTN_ENTITY_PRODUCT => __('WooCommerce Product'),
...
$likebtn_like_button_default_locales = array(
...
function likebtn_like_button_init() {
    _likebtn_like_button_plugin_on_load();
...
function likebtn_like_button_links($links, $file) {
    ...
    if (basename($file) == $plugin_file) {
        $settings_link = '<a href="admin.php?page=likebtn_like_button_settings">' . __('Settings', LIKEBTN_I18N_DOMAIN) . '</a>';
...
function likebtn_like_button_admin_menu() {
    ...
    add_menu_page(__('Like Buttons', LIKEBTN_I18N_DOMAIN), __('Like Buttons', LIKEBTN_I18N_DOMAIN), 'manage_options', 'likebtn_like_button_buttons', '', $logo_url);
    add_submenu_page(
            'likebtn_like_button_buttons', __('Buttons', LIKEBTN_I18N_DOMAIN) . ' ‹ ' . __('LikeBtn Like Button', LIKEBTN_I18N_DOMAIN), __('Buttons', LIKEBTN_I18N_DOMAIN), 'manage_options', 'likebtn_like_button_buttons', 'likebtn_like_button_admin_buttons'
    );
    ...
    add_submenu_page(
            'likebtn_like_button_buttons', __('Synchronization', LIKEBTN_I18N_DOMAIN) . ' ‹ ' . __('LikeBtn Like Button', LIKEBTN_I18N_DOMAIN), __('Synchronization', LIKEBTN_I18N_DOMAIN), 'manage_options', 'likebtn_like_button_settings', 'likebtn_like_button_admin_settings'
    );
    add_submenu_page(
            'likebtn_like_button_buttons', __('Statistics', LIKEBTN_I18N_DOMAIN) . ' ‹ LikeBtn Like Button', __('Statistics', LIKEBTN_I18N_DOMAIN), 'manage_options', 'likebtn_like_button_statistics', 'likebtn_like_button_admin_statistics'
    );
    add_submenu_page(
...
//        </div>
    ...
    if ( (($installation_timestamp && strtotime(date('Y-m-d H:i:s', $installation_timestamp) . ' +' . LIKEBTN_REVIEW_LINK_PERIOD) < time())
         ...
         && (get_option('likebtn_like_button_plan') != LIKEBTN_PLAN_FREE && get_option('likebtn_like_button_plan') != LIKEBTN_PLAN_TRIAL)
    ) {*/
    ...
    $header .= '
        <h2 class="nav-tab-wrapper">
            <a class="nav-tab ' . ($_GET['page'] == 'likebtn_like_button_buttons' ? 'nav-tab-active' : '') . '" href="' . admin_url() . 'admin.php?page=likebtn_like_button_buttons">' . __('Buttons', LIKEBTN_I18N_DOMAIN) . '</a>
            <a class="nav-tab ' . ($_GET['page'] == 'likebtn_like_button_settings' ? 'nav-tab-active' : '') . '" href="' . admin_url() . 'admin.php?page=likebtn_like_button_settings">' . __('Synchronization', LIKEBTN_I18N_DOMAIN) . '</a>
            <a class="nav-tab ' . ($_GET['page'] == 'likebtn_like_button_statistics' ? 'nav-tab-active' : '') . '" href="' . admin_url() . 'admin.php?page=likebtn_like_button_statistics">' . __('Statistics', LIKEBTN_I18N_DOMAIN) . ' <i class="premium_feature" title="PRO / VIP / ULTRA">&nbsp;</i></a>
...
function likebtn_like_button_activation_hook() {
    ...
    // Install DB
    _likebtn_like_button_db_install();

    $likebtn_like_button_entities = _likebtn_like_button_get_entities();

    // set default values for options
    add_option('likebtn_like_button_plan', LIKEBTN_PLAN_TRIAL);
...
function _likebtn_like_button_db_install() {
    global $wpdb;
...
// Plugin loaded
function _likebtn_like_button_plugin_on_load()
{
    _likebtn_like_button_db_update();

    _likebtn_like_button_update_options();
...
// Update DB
function _likebtn_like_button_db_update() {
    $db_version = (int)get_option('likebtn_like_button_db_version');
    if (!$db_version) {
        add_option('likebtn_like_button_db_version', LIKEBTN_DB_VERSION);
    }
...
// update options
function _likebtn_like_button_update_options() {
    $likebtn_like_button_entities = _likebtn_like_button_get_entities();
    ...
    foreach ($likebtn_like_button_entities as $entity_name => $entity_title) {
        ...
        if (!get_option('likebtn_like_button_settings_lang_' . $entity_name)) {
            _likebtn_like_button_set_default_options($entity_name);
        }
        ...
        if (_likebtn_has_list_flag($entity_name)) {
            ...
            if (get_option('likebtn_like_button_post_view_mode_' . $entity_name) && get_option('likebtn_like_button_show_' . $original_name) == '1') {
                ...
                {
                    update_option('likebtn_like_button_show_' . $entity_name, '1');
                    update_option('likebtn_like_button_use_settings_from_' . $entity_name, $original_name);
                }
                delete_option('likebtn_like_button_post_view_mode_' . $entity_name);
            }
        }
...
function _likebtn_like_button_set_default_options($entity_name) {
    ...
    if ($entity_name != LIKEBTN_ENTITY_COMMENT) {
        add_option('likebtn_like_button_show_' . $entity_name, '1');
    } else {
    ...
    if (_likebtn_has_list_flag($entity_name)) {
        add_option('likebtn_like_button_use_settings_from_' . $entity_name, _likebtn_cut_list_flag($entity_name));
    } else {
        add_option('likebtn_like_button_use_settings_from_' . $entity_name, '');
    }

    //add_option('likebtn_like_button_post_view_mode_' . $entity_name, LIKEBTN_POST_VIEW_MODE_BOTH);
    ...
    if ($entity_name == LIKEBTN_ENTITY_PRODUCT || $entity_name == LIKEBTN_ENTITY_EXCERPT) {
        add_option('likebtn_like_button_position_' . $entity_name, LIKEBTN_POSITION_TOP);
    } else {
        add_option('likebtn_like_button_position_' . $entity_name, LIKEBTN_POSITION_BOTTOM);
    }
    add_option('likebtn_like_button_alignment_' . $entity_name, LIKEBTN_ALIGNMENT_LEFT);
...
function likebtn_like_button_admin_settings() {
    ...
    <div id="poststuff" class="metabox-holder has-right-sidebar likebtn_like_button_subpage">
        <form method="post" action="options.php">
            ...
            <p class="description">
                <?php _e('Enter this information in order to enable synchronization of likes from LikeBtn.com system into your database and get the opportunity to use the following features:', LIKEBTN_I18N_DOMAIN); ?><br/>
                ● <?php _e('View statistics on Statistics tab.', LIKEBTN_I18N_DOMAIN); ?><br/>
                ● <?php _e('Sort posts and comments by votes.', LIKEBTN_I18N_DOMAIN); ?><br/>
                ● <?php _e('Use Most Liked Content widget.', LIKEBTN_I18N_DOMAIN); ?><br/>
            </p>
            <br/>
                        ...
                        <tr valign="top">
                            ...
                            <td>
                                <select name="likebtn_like_button_sync_inerval" <?php disabled((!get_option('likebtn_like_button_account_email') || !get_option('likebtn_like_button_account_api_key'))); ?> >
                                    <option value="" <?php selected('', get_option('likebtn_like_button_sync_inerval')); ?> ><?php _e('Do not fetch votes from LikeBtn.com into my database', LIKEBTN_I18N_DOMAIN) ?></option>
                                    <?php foreach ($likebtn_like_button_sync_intervals as $sync_interval): ?>
                                        <option value="<?php echo $sync_interval; ?>" <?php selected($sync_interval, get_option('likebtn_like_button_sync_inerval')); ?> ><?php echo $sync_interval; ?> <?php _e('min', LIKEBTN_I18N_DOMAIN); ?></option>
                                    <?php endforeach ?>
                                </select>
                                <br/><br/>
                                <input class="button-primary" type="button" value="<?php _e('Test sync', LIKEBTN_I18N_DOMAIN); ?>" onclick="testSync('<?php echo _likebtn_like_button_get_public_url() ?>img/ajax_loader.gif')" /> &nbsp;<strong class="likebtn_like_button_test_sync_container"><img src="<?php echo _likebtn_like_button_get_public_url() ?>img/ajax_loader.gif" class="hidden"/></strong>
                                <br/><br/>
                                <input class="button-secondary likebtn_ttip" type="button" value="<?php _e('Run full sync manually', LIKEBTN_I18N_DOMAIN); ?>" onclick="manualSync('<?php echo _likebtn_like_button_get_public_url() ?>img/ajax_loader.gif')" title="<?php _e("ATTENTION: Use this feature carefully since full synchronization may affect your website performance. If you don't experience any problems with likes synchronization better to avoid using this feature.", LIKEBTN_I18N_DOMAIN) ?>" /> &nbsp;<strong class="likebtn_like_button_manual_sync_container"><img src="<?php echo _likebtn_like_button_get_public_url() ?>img/ajax_loader.gif" class="hidden"/></strong>
                            </td>
                        </tr>
                    </table>
                </div>
...
function likebtn_like_button_admin_buttons() {
    ...
    if (!$locales) {
        $locales = $likebtn_like_button_default_locales;
    }
                                    ...
                                    <td>
                                        <select name="likebtn_like_button_use_settings_from_<?php echo $entity_name; ?>" onChange="userSettingsFromChange(this, '<?php echo $entity_name; ?>')">
                                            ...
                                            <?php foreach ($likebtn_like_button_entities as $use_entity_name => $use_entity_title): ?>
                                                ...
                                                ?>
                                                <option value="<?php echo $use_entity_name; ?>" <?php selected($use_entity_name, get_option('likebtn_like_button_use_settings_from_' . $entity_name)); ?> ><?php _e($use_entity_title, LIKEBTN_I18N_DOMAIN); ?></option>
                                            <?php endforeach ?>
                                        </select>
                                    </td>
                                </tr>
                                    ...
                                    <div class="inside">
                                        ...
                                        <table class="form-table">
                                            ...
                                            <tr valign="top">
                                                ...
                                                <td>
                                                    <select name="likebtn_like_button_settings_counter_type_<?php echo $entity_name; ?>">
                                                        <option value="number" <?php selected('number', get_option('likebtn_like_button_settings_counter_type_' . $entity_name)); ?> ><?php _e('Number', LIKEBTN_I18N_DOMAIN); ?></option>
                                                        <option value="percent" <?php selected('percent', get_option('likebtn_like_button_settings_counter_type_' . $entity_name)); ?> ><?php _e('Percent', LIKEBTN_I18N_DOMAIN); ?></option>
                                                        <option value="subtract_dislikes" <?php selected('subtract_dislikes', get_option('likebtn_like_button_settings_counter_type_' . $entity_name)); ?> ><?php _e('Subtract dislikes', LIKEBTN_I18N_DOMAIN); ?></option>
                                                        <option value="single_number" <?php selected('single_number', get_option('likebtn_like_button_settings_counter_type_' . $entity_name)); ?> ><?php _e('Next to Dislike button', LIKEBTN_I18N_DOMAIN); ?></option>
                                                    </select>
                                                </td>
                                            </tr>
                                            <tr valign="top">
                                            ...
                                            <tr valign="top">
                                                ...
                                                <td>
                                                    <input type="radio" name="likebtn_like_button_position_<?php echo $entity_name; ?>" value="<?php echo LIKEBTN_POSITION_TOP ?>" <?php if (LIKEBTN_POSITION_TOP == get_option('likebtn_like_button_position_' . $entity_name)): ?>checked="checked"<?php endif ?> /> <?php _e('Top of Content', LIKEBTN_I18N_DOMAIN); ?>&nbsp;&nbsp;&nbsp;
                                                    <input type="radio" name="likebtn_like_button_position_<?php echo $entity_name; ?>" value="<?php echo LIKEBTN_POSITION_BOTTOM ?>" <?php if (LIKEBTN_POSITION_BOTTOM == get_option('likebtn_like_button_position_' . $entity_name) || !get_option('likebtn_like_button_position_' . $entity_name)): ?>checked="checked"<?php endif ?> /> <?php _e('Bottom of Content', LIKEBTN_I18N_DOMAIN); ?>&nbsp;&nbsp;&nbsp;
                                                    <input type="radio" name="likebtn_like_button_position_<?php echo $entity_name; ?>" value="<?php echo LIKEBTN_POSITION_BOTH ?>" <?php if (LIKEBTN_POSITION_BOTH == get_option('likebtn_like_button_position_' . $entity_name)): ?>checked="checked"<?php endif ?> /> <?php _e('Top and Bottom', LIKEBTN_I18N_DOMAIN); ?>
                                            ...
                                            <?php if (!in_array($entity_name, array(LIKEBTN_ENTITY_PRODUCT))): ?>
                                                <tr valign="top">
                                                    ...
                                                    <td>
                                                        <input type="radio" name="likebtn_like_button_alignment_<?php echo $entity_name; ?>" value="<?php echo LIKEBTN_ALIGNMENT_LEFT; ?>" <?php if (LIKEBTN_ALIGNMENT_LEFT == get_option('likebtn_like_button_alignment_' . $entity_name) || !get_option('likebtn_like_button_alignment_' . $entity_name)): ?>checked="checked"<?php endif ?> /> <?php _e('Left'); ?>
                                                        &nbsp;&nbsp;&nbsp;
                                                        <input type="radio" name="likebtn_like_button_alignment_<?php echo $entity_name; ?>" value="<?php echo LIKEBTN_ALIGNMENT_CENTER; ?>" <?php if (LIKEBTN_ALIGNMENT_CENTER == get_option('likebtn_like_button_alignment_' . $entity_name)): ?>checked="checked"<?php endif ?> /> <?php _e('Center'); ?>
                                                        &nbsp;&nbsp;&nbsp;
                                                        <input type="radio" name="likebtn_like_button_alignment_<?php echo $entity_name; ?>" value="<?php echo LIKEBTN_ALIGNMENT_RIGHT; ?>" <?php if (LIKEBTN_ALIGNMENT_RIGHT == get_option('likebtn_like_button_alignment_' . $entity_name)): ?>checked="checked"<?php endif ?> /> <?php _e('Right'); ?>
                                        ...
                                        <div class="postbox">
                                            ...
                                            <h3 style="cursor:pointer" onclick="toggleCollapsable(this)" class="likebtn_like_button_collapse_trigger"><small>►</small> <?php _e('Extended Settings', LIKEBTN_I18N_DOMAIN); ?></h3>
                                            <div class="inside hidden">
                                                ...
                                                <div class="postbox">
                                                    <h3 style="cursor:pointer" onclick="toggleCollapsable(this)" class="likebtn_like_button_collapse_trigger"><small>►</small> <?php _e('General', LIKEBTN_I18N_DOMAIN); ?></h3>
                                                    <div class="inside hidden">
                                                        <table class="form-table" >
                                                            <?php /*if (!in_array($entity_name, $likebtn_like_button_no_excerpts)): ?>
                                                                <tr valign="top">
                                                                    <th scope="row"><label><?php _e('View mode', LIKEBTN_I18N_DOMAIN); ?></label></th>
                                                                    <td>
                                                                        <input type="radio" name="likebtn_like_button_post_view_mode_<?php echo $entity_name; ?>" value="<?php echo LIKEBTN_POST_VIEW_MODE_FULL; ?>" <?php checked(LIKEBTN_POST_VIEW_MODE_FULL, get_option('likebtn_like_button_post_view_mode_' . $entity_name)) ?> /> <?php _e('Full', LIKEBTN_I18N_DOMAIN); ?>&nbsp;&nbsp;&nbsp;
                                                                        <input type="radio" name="likebtn_like_button_post_view_mode_<?php echo $entity_name; ?>" value="<?php echo LIKEBTN_POST_VIEW_MODE_EXCERPT; ?>" <?php checked(LIKEBTN_POST_VIEW_MODE_EXCERPT, get_option('likebtn_like_button_post_view_mode_' . $entity_name)) ?> /> <?php _e('Excerpt', LIKEBTN_I18N_DOMAIN); ?>&nbsp;&nbsp;&nbsp;
                                                                        <input type="radio" name="likebtn_like_button_post_view_mode_<?php echo $entity_name; ?>" value="<?php echo LIKEBTN_POST_VIEW_MODE_BOTH; ?>" <?php checked(LIKEBTN_POST_VIEW_MODE_BOTH, get_option('likebtn_like_button_post_view_mode_' . $entity_name)) ?> /> <?php _e('Both', LIKEBTN_I18N_DOMAIN); ?>
                                                            ...
                                                            <tr valign="top">
                                                                ...
                                                                <td>
                                                                    <input type="checkbox" name="likebtn_like_button_post_format_<?php echo $entity_name; ?>[]" value="all" <?php if (in_array('all', get_option('likebtn_like_button_post_format_' . $entity_name))): ?>checked="checked"<?php endif ?> onClick="postFormatAllChange(this, '<?php echo $entity_name; ?>')" /> <?php _e('All', LIKEBTN_I18N_DOMAIN); ?>&nbsp;&nbsp;&nbsp;
                                                                    <span id="post_format_container_<?php echo $entity_name; ?>" <?php if (in_array('all', get_option('likebtn_like_button_post_format_' . $entity_name))): ?>style="display:none"<?php endif ?>>
                                                                        <?php foreach ($post_formats as $post_format): ?>
                                                                            <input type="checkbox" name="likebtn_like_button_post_format_<?php echo $entity_name; ?>[]" value="<?php echo $post_format; ?>" <?php if (in_array($post_format, get_option('likebtn_like_button_post_format_' . $entity_name))): ?>checked="checked"<?php endif ?> /> <?php _e(__(ucfirst($post_format), LIKEBTN_I18N_DOMAIN)); ?>&nbsp;&nbsp;&nbsp;
                                                                        <?php endforeach ?>
                                                                    </span>
                                                                </td>
                                                            </tr>
                                                            ...
                                                            <tr valign="top">
                                                                ...
                                                                <td>
                                                                    ...
                                                                    <input type="checkbox" name="likebtn_like_button_exclude_sections_<?php echo $entity_name; ?>[]" value="archive" <?php if (in_array('archive', $excluded_sections)): ?>checked="checked"<?php endif ?> /> <?php _e('Archive', LIKEBTN_I18N_DOMAIN); ?>
                                                                </td>
                                                            </tr>
                                                            <tr valign="top">
                                                            ...
                                                            <tr valign="top">
                                                                ...
                                                                <td>
                                                                    <input type="radio" name="likebtn_like_button_user_logged_in_<?php echo $entity_name; ?>" value="1" <?php checked('1', get_option('likebtn_like_button_user_logged_in_' . $entity_name)) ?> /> <?php _e('Logged in', LIKEBTN_I18N_DOMAIN); ?>&nbsp;&nbsp;&nbsp;
                                                                    <input type="radio" name="likebtn_like_button_user_logged_in_<?php echo $entity_name; ?>" value="0" <?php checked('0', get_option('likebtn_like_button_user_logged_in_' . $entity_name)) ?> /> <?php _e('Not logged in', LIKEBTN_I18N_DOMAIN); ?>&nbsp;&nbsp;&nbsp;
                                                                    <input type="radio" name="likebtn_like_button_user_logged_in_<?php echo $entity_name; ?>" value="" <?php checked('', get_option('likebtn_like_button_user_logged_in_' . $entity_name)) ?> /> <?php _e('For all', LIKEBTN_I18N_DOMAIN); ?>
                                                ...
                                                <div class="postbox">
                                                    <h3 style="cursor:pointer" onclick="toggleCollapsable(this)" class="likebtn_like_button_collapse_trigger"><small>►</small> <?php _e('Voting', LIKEBTN_I18N_DOMAIN); ?></h3>
                                                    <div class="inside hidden">
                                                        <table class="form-table">
                                                            <tr valign="top">
                                                            ...
                                                            <tr valign="top">
                                                                ...
                                                                <td>
                                                                    <select name="likebtn_like_button_settings_revote_period_<?php echo $entity_name; ?>">
                                                                        ...
                                                                        <option value="60" <?php selected('1', get_option('likebtn_like_button_settings_popup_position_' . $entity_name)); ?> ><?php _e('Every second', LIKEBTN_I18N_DOMAIN); ?> *</option>
                                                                        <option value="60" <?php selected('60', get_option('likebtn_like_button_settings_popup_position_' . $entity_name)); ?> ><?php _e('Every minute', LIKEBTN_I18N_DOMAIN); ?> *</option>
                                                                        <option value="3600" <?php selected('3600', get_option('likebtn_like_button_settings_popup_position_' . $entity_name)); ?> ><?php _e('Hourly', LIKEBTN_I18N_DOMAIN); ?></option>
                                                                        <option value="86400" <?php selected('86400', get_option('likebtn_like_button_settings_popup_position_' . $entity_name)); ?> ><?php _e('Daily', LIKEBTN_I18N_DOMAIN); ?></option>
                                                                        <option value="604800" <?php selected('604800', get_option('likebtn_like_button_settings_popup_position_' . $entity_name)); ?> ><?php _e('Weekly', LIKEBTN_I18N_DOMAIN); ?></option>
                                                                        <option value="2592000" <?php selected('2592000', get_option('likebtn_like_button_settings_popup_position_' . $entity_name)); ?> ><?php _e('Monthly', LIKEBTN_I18N_DOMAIN); ?></option>
                                                                        <option value="31536000" <?php selected('31536000', get_option('likebtn_like_button_settings_popup_position_' . $entity_name)); ?> ><?php _e('Annually', LIKEBTN_I18N_DOMAIN); ?></option>
                                                                    </select>
                                                                </td>
                                                            </tr>
                                                        </table>
                                                ...
                                                <div class="postbox">
                                                    <h3 style="cursor:pointer" onclick="toggleCollapsable(this)" class="likebtn_like_button_collapse_trigger"><small>►</small> <?php _e('Counter', LIKEBTN_I18N_DOMAIN); ?></h3>
                                                    <div class="inside hidden">
                                                        <table class="form-table">
                                                         <tr valign="top">
                                                ...
                                                <div class="postbox">
                                                    <h3 style="cursor:pointer" onclick="toggleCollapsable(this)" class="likebtn_like_button_collapse_trigger"><small>►</small> <?php _e('Popup', LIKEBTN_I18N_DOMAIN); ?></h3>
                                                    <div class="inside hidden">
                                                        <table class="form-table">
                                                            <tr valign="top">
                                                            ...
                                                            <tr valign="top">
                                                                ...
                                                                <td>
                                                                    <select name="likebtn_like_button_settings_popup_position_<?php echo $entity_name; ?>">
                                                                        <option value="top" <?php selected('top', get_option('likebtn_like_button_settings_popup_position_' . $entity_name)); ?> ><?php _e('top', LIKEBTN_I18N_DOMAIN); ?></option>
                                                                        <option value="right" <?php selected('right', get_option('likebtn_like_button_settings_popup_position_' . $entity_name)); ?> ><?php _e('right', LIKEBTN_I18N_DOMAIN); ?></option>
                                                                        <option value="bottom" <?php selected('bottom', get_option('likebtn_like_button_settings_popup_position_' . $entity_name)); ?> ><?php _e('bottom', LIKEBTN_I18N_DOMAIN); ?></option>
                                                                        <option value="left" <?php selected('left', get_option('likebtn_like_button_settings_popup_position_' . $entity_name)); ?> ><?php _e('left', LIKEBTN_I18N_DOMAIN); ?></option>
                                                                    </select>
                                                                </td>
                                                            </tr>
                                                            <tr valign="top">
                                                                ...
                                                                <td>
                                                                    <select name="likebtn_like_button_settings_popup_style_<?php echo $entity_name; ?>">
                                                                        <option value="light" <?php selected('light', get_option('likebtn_like_button_settings_popup_style_' . $entity_name)); ?> ><?php _e('light', LIKEBTN_I18N_DOMAIN); ?></option>
                                                                        <option value="dark" <?php selected('dark', get_option('likebtn_like_button_settings_popup_style_' . $entity_name)); ?> ><?php _e('dark', LIKEBTN_I18N_DOMAIN); ?></option>
                                                                    </select>
                                                                </td>
                                                            </tr>
                                                            <tr valign="top">
                                                ...
                                                <?php /*<div class="postbox">
                                                    <h3 style="cursor:pointer" onclick="toggleCollapsable(this)" class="likebtn_like_button_collapse_trigger"><small>►</small> <?php _e('Sharing', LIKEBTN_I18N_DOMAIN); ?></h3>
                                                    <div class="inside hidden">
                                                        <table class="form-table">
                                                ...
                                                <div class="postbox">
                                                    <h3 style="cursor:pointer" onclick="toggleCollapsable(this)" class="likebtn_like_button_collapse_trigger"><small>►</small> <?php _e('Loading', LIKEBTN_I18N_DOMAIN); ?></h3>
                                                    <div class="inside hidden">
                                                        <table class="form-table">
                                                            <tr valign="top">
                                                ...
                                                <?php /*<div class="postbox">
                                                    <h3 style="cursor:pointer" onclick="toggleCollapsable(this)" class="likebtn_like_button_collapse_trigger"><small>►</small> <?php _e('Domains', LIKEBTN_I18N_DOMAIN); ?></h3>
                                                    <div class="inside hidden">
                                                        <table class="form-table">
                                                            <tr valign="top">
                                                ...
                                                <div class="postbox">
                                                    <h3 style="cursor:pointer" onclick="toggleCollapsable(this)" class="likebtn_like_button_collapse_trigger"><small>►</small> <?php _e('Events & messages', LIKEBTN_I18N_DOMAIN); ?></h3>
                                                    <div class="inside hidden">
                                                        <table class="form-table">
                                                            <tr valign="top">
                                                                <th scope="row">
                                                ...
                                                <div class="postbox">
                                                    <h3 style="cursor:pointer" onclick="toggleCollapsable(this)" class="likebtn_like_button_collapse_trigger"><small>►</small> <?php _e('Texts', LIKEBTN_I18N_DOMAIN); ?></h3>
                                                    <div class="inside hidden">
                                                        <table class="form-table">
                                                            <tr valign="top">
...
function likebtn_like_button_admin_statistics() {
    ...
    // Custom item
    $likebtn_like_button_entities[LIKEBTN_ENTITY_CUSTOM_ITEM] = __('Custom item');
    ...
    $likebtn_like_button_post_statuses['0'] = __('Comment not approved', LIKEBTN_I18N_DOMAIN);
    $likebtn_like_button_post_statuses['1'] = __('Comment approved', LIKEBTN_I18N_DOMAIN);
                ...
                <?php foreach ($statistics as $statistics_item): ?>
                    ...
                    <tr id="item_<?php echo $statistics_item->post_id; ?>">
                        ...
                        <td>
                            ...
                            <?php else: ?>
                                <a href="javascript:statisticsEdit('<?php echo $entity_name ?>', '<?php echo $statistics_item->post_id; ?>', 'like', '<?php echo get_option('likebtn_like_button_plan'); ?>', '<?php _e('Enter new value:', LIKEBTN_I18N_DOMAIN) ?>', '<?php _e('Upgrade your website plan to the ULTRA plan to use the feature', LIKEBTN_I18N_DOMAIN) ?>', '<?php _e('Error occured. Please, try again later.', LIKEBTN_I18N_DOMAIN) ?>');void(0);" title="<?php _e('Click to change', LIKEBTN_I18N_DOMAIN) ?>" class="item_like"><?php echo $statistics_item->likes; ?></a>
                            <?php endif ?>
                        </td>
                        <td>
                            <?php if ($blogs && $statistics_item->blog_id != $blog_id): ?>
                            ...
                            <?php else: ?>
                                <a href="javascript:statisticsEdit('<?php echo $entity_name ?>', '<?php echo $statistics_item->post_id; ?>', 'dislike', '<?php echo get_option('likebtn_like_button_plan'); ?>', '<?php _e('Enter new value:', LIKEBTN_I18N_DOMAIN) ?>', '<?php _e('Upgrade your website plan to the ULTRA plan to use the feature', LIKEBTN_I18N_DOMAIN) ?>', '<?php _e('Error occured. Please, try again later.', LIKEBTN_I18N_DOMAIN) ?>');void(0);" title="<?php _e('Click to change', LIKEBTN_I18N_DOMAIN) ?>" class="item_dislike"><?php echo $statistics_item->dislikes; ?></a>
                            <?php endif ?>
                        </td>
...
// Get entity types
function _likebtn_like_button_get_entities($no_list = false) {
    /*global $likebtn_like_button_entities;
    ...
    $likebtn_like_button_entities = array(
        LIKEBTN_ENTITY_POST => _likebtn_like_button_get_entity_title(LIKEBTN_ENTITY_POST),
        LIKEBTN_ENTITY_POST_LIST => _likebtn_like_button_get_entity_title(LIKEBTN_ENTITY_POST_LIST),
        LIKEBTN_ENTITY_PAGE => _likebtn_like_button_get_entity_title(LIKEBTN_ENTITY_PAGE),
    );
    ...
    if (!empty($post_types)) {
        foreach ($post_types as $post_type) {
            ...
            $likebtn_like_button_entities[$post_type.LIKEBTN_LIST_FLAG] = _likebtn_like_button_get_entity_title($post_type.LIKEBTN_LIST_FLAG);
        }
    }

    // append Comments
    $likebtn_like_button_entities[LIKEBTN_ENTITY_COMMENT] = _likebtn_like_button_get_entity_title(LIKEBTN_ENTITY_COMMENT);
...
function likebtn_like_button_get_content($content, $callback_content_position = '') {
...
function likebtn_like_button_the_content($content) {
    ...
    $content = likebtn_like_button_get_content($content);
    return $content;
}
...
function likebtn_like_button_woocommerce_product($content) {
    $content = likebtn_like_button_get_content($content);
    echo $content;
}
// WooCommerce - top
function likebtn_like_button_woocommerce_product_top($content) {
    $content = likebtn_like_button_get_content($content, '_likebtn_like_button_woocommerce_content_top');
    echo $content;
}
function _likebtn_like_button_woocommerce_content_top($content, $html, $position) {
    // WooCommerce
...
// WooCommerce - bottom
function likebtn_like_button_woocommerce_after_main_content($content) {
    if (!is_single()) {
        return false;
    }
    $content = likebtn_like_button_get_content($content, '_likebtn_like_button_woocommerce_content_bottom');
    echo $content;
}
function likebtn_like_button_woocommerce_product_bottom($content) {
    $content = likebtn_like_button_get_content($content, '_likebtn_like_button_woocommerce_content_bottom');
    echo $content;
}
function _likebtn_like_button_woocommerce_content_bottom($content, $html, $position) {
    // WooCommerce
    if ($position == LIKEBTN_POSITION_TOP) {
        return $content;
    } else {
...
add_action('woocommerce_single_product_summary', 'likebtn_like_button_woocommerce_product_top', 7);
add_action('woocommerce_after_main_content', 'likebtn_like_button_woocommerce_after_main_content', 7);
add_action('woocommerce_after_shop_loop_item_title', 'likebtn_like_button_woocommerce_product_top', 7);
add_action('woocommerce_after_shop_loop_item_title', 'likebtn_like_button_woocommerce_product_bottom', 12);
...
function likebtn_post($post_id = NULL) {
    ...
    if ($entity_name == LIKEBTN_ENTITY_POST && get_option('likebtn_like_button_show_' . LIKEBTN_ENTITY_POST) == '1') {
        return;
    }
    if ($entity_name == LIKEBTN_ENTITY_PAGE && get_option('likebtn_like_button_show_' . LIKEBTN_ENTITY_PAGE) == '1') {
        return;
    }
...
function likebtn_comment($comment_id = NULL) {
    ...
    if (get_option('likebtn_like_button_show_' . LIKEBTN_ENTITY_COMMENT) == '1') {
        return;
    }

    $html = _likebtn_like_button_get_markup(LIKEBTN_ENTITY_COMMENT, $comment_id);
...
function _likebtn_like_button_get_entity_title($entity_name)
{
    global $likebtn_like_button_entity_titles;
    ...
    if (!array_key_exists($entity_name, $likebtn_like_button_entity_titles)) {
        ...
        $title = __($likebtn_like_button_entity_titles[$entity_name]);
    }
