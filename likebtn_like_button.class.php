...
//        return true;
    ...
    public function runSyncLocales() {
        if ($this->timeToSyncLocales(LIKEBTN_LIKE_BUTTON_LOCALES_SYNC_INTERVAL)) {
            $this->syncLocales();
        }
    }
    ...
    public function timeToSyncLocales($sync_period) {
        ...
        $last_sync_time = get_option('likebtn_like_button_last_locale_sync_time');

        $now = time();
        if (!$last_sync_time) {
            update_option('likebtn_like_button_last_locale_sync_time', $now);
            return true;
        } else {
            if ($last_sync_time + $sync_period > $now) {
                return false;
    ...
    public function syncLocales() {
        ...
        $last_sync_time = number_format(get_option('likebtn_like_button_last_locale_sync_time'), 0, '', '');
